...
begin
  assume m n,
  assume eq,
  --rewrite eq, -> this works too.
  apply congr_arg succ,
  assumption,
  --ANY function f has this property. :)
  -- good for cancelling out
end
...
begin
  assume a b c d,
  rewrite add_assoc a b c, -- Match to the proof definition!
...
begin
  assume m n,
  -- Choose n as it is on the right side!
  induction n with n' ih,
  dsimp [add,double],
  refl,
...
begin
  ...
  -- Do i need induction?
  -- Which variable to use on induction?
  -- Use lemmas to simplify the proofs.
  rewrite add_succ_lem,
end

theorem double_comm : ∀ (m n : ℕ), 
    double (m + n) = double (n + m) :=
begin
  assume m n,
  rewrite add_comm,
end                   
